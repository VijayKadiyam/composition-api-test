      ...
      <b-row>
        ...
        >
          <b-button
            ...
            @click="store"
          >
            {{ loading ? 'Loading... ' : 'Update Circular' }}
          </b-button>
        </b-col>
      </b-row>
    </b-form>
...
export default {
  ...
  methods: {
    async getData() {
      const circular = await axios.get(`/circulars/${this.$route.params.circularId}`)
      this.form = circular.data.data
    },
    async store() {
      try {
        this.loading = true
        const circular = await axios.patch(`/circulars/${this.$route.params.circularId}`, this.form)
