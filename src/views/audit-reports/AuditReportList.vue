        ...
        <b-row>
          ...
          >
            <div class="d-flex">
              <b-button
                variant="primary"
                href="/audit-reports/create"
              >
                <span class="text-nowrap">Add Audit Report</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      >
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item :to="`/audit-reports/${data.item.id}`">
...
export default {
  ...
  methods: {
    async getData() {
      this.items = []
      this.loading = true
      const items = await axios.get('/audit_reports')
      this.items = items.data.data
      this.loading = false
    },
    async del(id) {
      const r = window.confirm('Are you sure you want to delete the data?')
      if (r) await axios.delete(`/audit_reports/${id}`)
      this.getData()
    },
  },
}
