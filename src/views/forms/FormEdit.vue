...
export default {
  ...
  methods: {
    async getData() {
      const form = await axios.get(`/forms/${this.$route.params.formId}`)
      this.form = form.data.data
    },
    async update() {
      try {
        this.loading = true
        const form = await axios.patch(`/forms/${this.$route.params.formId}`, this.form)
        this.form.id = form.data.data.id
    ...
    async handleFileUpload() {
      ...
      await axios.post('upload_form_attachment', formData,
        {
          headers: {
