      ...
      <b-row>
        ...
        >
          <b-button
            ...
            @click="store"
          >
            {{ loading ? 'Loading... ' : 'Update Library' }}
          </b-button>
        </b-col>
      </b-row>
    </b-form>
...
export default {
  ...
  methods: {
    async getData() {
      const library = await axios.get(`/libraries/${this.$route.params.libraryId}`)
      this.form = library.data.data
    },
    async store() {
      try {
        this.loading = true
        const library = await axios.patch(`/libraries/${this.$route.params.libraryId}`, this.form)
