        ...
        <b-row>
          ...
          >
            <div class="d-flex">
              <b-button
                variant="primary"
                href="/sire-reports/create"
              >
                <span class="text-nowrap">Add Sire Report</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      >
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item :to="`/sire-reports/${data.item.id}`">
...
export default {
  ...
  methods: {
    async getData() {
      this.items = []
      this.loading = true
      const items = await axios.get('/sire_reports')
      this.items = items.data.data
      this.loading = false
    },
    async del(id) {
      const r = window.confirm('Are you sure you want to delete the data?')
      if (r) await axios.delete(`/sire_reports/${id}`)
      this.getData()
    },
  },
}
