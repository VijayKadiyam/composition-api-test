        ...
        <b-row>
          ...
          >
            <div class="d-flex">
              ...
              >
                <span class="text-nowrap">Add Screening</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      >
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item :to="`/screenings/${data.item.id}`">
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        { key: 'actions' },
        { key: 'ship_name', sortable: true },
        { key: 'client_name', sortable: true },
        { key: 'report_number', sortable: true },
  ...
  methods: {
    async getData() {
      this.items = []
      this.loading = true
      const items = await axios.get('/screenings')
      this.items = items.data.data
      this.loading = false
    },
    async del(id) {
      const r = window.confirm('Are you sure you want to delete the data?')
      if (r) await axios.delete(`/screenings/${id}`)
      this.getData()
    },
  },
}
