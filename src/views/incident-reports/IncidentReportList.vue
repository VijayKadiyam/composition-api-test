        ...
        <b-row>
          ...
          >
            <div class="d-flex">
              ...
              >
                <span class="text-nowrap">Add Incident Report</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      >
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item :to="`/incident-reports/${data.item.id}`">
...
export default {
  ...
  methods: {
    async getData() {
      this.items = []
      this.loading = true
      const items = await axios.get('/incident_reports')
      this.items = items.data.data
      this.loading = false
    },
    async del(id) {
      const r = window.confirm('Are you sure you want to delete the data?')
      if (r) await axios.delete(`/incident_reports/${id}`)
      this.getData()
    },
  },
}
