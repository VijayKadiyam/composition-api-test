...
export default {
  ...
  methods: {
    async getData() {
      const manual = await axios.get(`/procedures/${this.$route.params.procedureId}`)
      this.form = manual.data.data
    },
    async update() {
      try {
        this.loading = true
        const manual = await axios.patch(`/procedures/${this.$route.params.procedureId}`, this.form)
    ...
    async handleFileUpload() {
      ...
      await axios.post('upload_procedure_attachment', formData,
        {
          headers: {
