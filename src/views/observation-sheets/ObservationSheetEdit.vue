      ...
      <b-row>
        ...
        >
          <b-button
            ...
            @click="store"
          >
            {{ loading ? 'Loading... ' : 'Update Observation Sheet' }}
          </b-button>
        </b-col>
      </b-row>
    </b-form>
...
export default {
  ...
  methods: {
    async getData() {
      const observationSheet = await axios.get(`/observation_sheets/${this.$route.params.observationSheetId}`)
      this.form = observationSheet.data.data
    },
    async store() {
      try {
        this.loading = true
        const observationSheet = await axios.patch(`/observation_sheets/${this.$route.params.observationSheetId}`, this.form)
