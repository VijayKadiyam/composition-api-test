...
export default {
  ...
  methods: {
    async getData() {
      const kpi = await axios.get(`/kpis/${this.$route.params.kpiId}`)
      this.form = kpi.data.data
    },
    async store() {
      try {
        this.loading = true
        const kpi = await axios.patch(`/kpis/${this.$route.params.kpiId}`, this.form)
        this.form.id = kpi.data.data.id
    ...
    async handleFileUpload() {
      ...
      await axios.post('upload_kpi_attachment', formData,
        {
          headers: {
